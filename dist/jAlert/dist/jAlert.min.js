! function($) {
    var timer;
    Date.now || (Date.now = function() { return +new Date }), $.fn.jAlert = function(options) {
        $("body").focus(), $("body").blur();
        var alert_id, sizeAliases = { xsmall: "xsm", small: "sm", medium: "md", large: "lg", xlarge: "xlg" },
            styles = [],
            classes = ["animated"],
            backgroundClasses = [];
        if (1 < this.length) return this.each(function() { $.fn.jAlert(options) }), this;
        ...
        alert.picture && (alert.image = alert.picture), alert.image ? (alert.noPadContent = !0, alert.content = "<div class='ja_media_wrap'>" + loader + "<img src='" + alert.image + "' class='ja_img' onload='$.fn.jAlert.mediaLoaded($(this))''", alert.imageWidth && (alert.content += " style='width: " + alert.imageWidth + "'"), alert.content += "></div>", alert.onOpen.unshift(function(elem) {
            var img_elem = elem.find(".ja_img");
            img_elem.on("load", function() { elem.css("width", img_elem.width() + "px"), elem.css("height", img_elem.height() + "px") })
        })) : alert.video ? (alert.content = "<div class='ja_media_wrap'>" + loader + "<div class='ja_video'></div></div>", alert.onOpen.unshift(function(elem) {
            var iframe = document.createElement("iframe");
            iframe.src = elem.data("jAlert").video, iframe.addEventListener ? iframe.addEventListener("load", function() { $.fn.jAlert.mediaLoaded($(this)) }, !0) : iframe.attachEvent ? iframe.attachEvent("onload", function() { $.fn.jAlert.mediaLoaded($(this)) }) : iframe.onload = function() { $.fn.jAlert.mediaLoaded($(this)) }, elem.find(".ja_video").append(iframe)
        })) : alert.iframe ? (alert.iframeHeight || classes.push("ja_stretch_height"), alert.content = "<div class='ja_media_wrap'>" + loader + "</div>", alert.onOpen.unshift(function(elem) {
            var iframe = document.createElement("iframe");
            iframe.src = elem.jAlert().iframe, iframe.className = "ja_iframe", iframe.addEventListener ? iframe.addEventListener("load", function() { $.fn.jAlert.mediaLoaded($(this)) }, !0) : iframe.attachEvent ? iframe.attachEvent("onload", function() { $.fn.jAlert.mediaLoaded($(this)) }) : iframe.onload = function() { $.fn.jAlert.mediaLoaded($(this)) }, elem.find(".ja_media_wrap").append(iframe)
        })) : alert.ajax && (alert.content = "<div class='ja_media_wrap'>" + loader + "</div>", onAjaxCallbacks = alert.onOpen, alert.onOpen = [function(elem) { $.ajax(elem.jAlert().ajax, { async: !0, complete: function(jqXHR, textStatus) { elem.find(".ja_media_wrap").replaceWith(jqXHR.responseText), $.each(onAjaxCallbacks, function(index, onAjax) { onAjax(elem) }) }, error: function(jqXHR, textStatus, errorThrown) { alert.onAjaxFail(elem, "Error getting content: Code: " + jqXHR.status + " : Msg: " + jqXHR.statusText) } }) }]);

        function getBtnHTML(btn) {
            var unique;
            return void 0 === btn.href && (btn.href = ""), void 0 === btn.class && (btn.class = ""), void 0 === btn.theme ? btn.class += " ja_btn_default" : btn.class += " ja_btn_" + btn.theme, void 0 === btn.text && (btn.text = ""), void 0 === btn.id && (unique = Date.now().toString() + Math.floor(1e5 * Math.random()), btn.id = "ja_btn_" + unique), void 0 === btn.target && (btn.target = "_self"), void 0 === btn.closeAlert && (btn.closeAlert = !0), $("body").off("click", "#" + btn.id), $("body").on("click", "#" + btn.id, function(e) {
                var button = $(this);
